...
- 2026-06-17 — **Canonical SHA superseded by [Proposal 031](031-s24-brent-s25-stale-core-macro.md)** (S-24 brent fetch wire-up). Adding `DCOILBRENTEU` to `config/gates.yml` → `data_staleness.series` changed the canonical content hash to **`a8d90addea00c75cfe114859707353b3483a24e1d1220321cbc84a744ef5e542`** (recomputed via `compute_gates_content_sha()` against the edited bytes). This is the value `TestArtefactSync` now asserts. The `89ff983d…` hash above remains the accurate record at 003's own DONE; this entry is append-only and does not alter it. Additive change only — no deployment-gate threshold moved.
- 2026-06-19 — **Canonical SHA superseded by [Proposal 032](032-s25c-eurostat-hicp-provider.md)** (S-25c euro-area HICP source migration). Replacing the dead ECB `ICP.M.U2.N.000000.4.ANR` key with the Eurostat `ei_cphi_m.TOTAL.RT12.EA` entry in `config/gates.yml` → `data_staleness.series` changed the canonical content hash to **`c437a02546412205c66a8df0bcc26864a7db04533e323514c6f5af85568cec96`** (recomputed via `compute_gates_content_sha()` against the edited bytes). This is the value `TestArtefactSync` now asserts. The `a8d90add…` hash above remains the accurate record at Proposal 031; this entry is append-only and does not alter it. Same-signal data-source migration — no deployment-gate threshold moved.
- 2026-06-25 — **Canonical SHA superseded by [Proposal 034](034-s24-equity-gate-manual-slot.md)** (S-24-remainder equity gate). Adding the `^STOXX` entry (source: YFINANCE) to `config/gates.yml` → `data_staleness.series` changed the canonical content hash to **`58b9151ba8c2db1f2e8b505d2cfe1e50f7563bcc87eeaee3ff7949be1938447e`** (recomputed via `compute_gates_content_sha()` against the edited bytes). This is the value `TestArtefactSync` now asserts. The `c437a025…` hash above remains the accurate record at Proposal 032; this entry is append-only and does not alter it. Additive data-source wiring — no deployment-gate threshold moved.
